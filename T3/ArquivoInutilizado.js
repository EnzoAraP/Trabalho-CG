...
let cacodemons_carregados = false;
...
let assetManager = {
   ...
   checkLoaded: function () {
      if (!this.allLoaded) {
         this.allLoaded = true;
         for (var i = 0; this.allLoaded && i < this.num_total; i++) {
            if (!this.cacodemon)
...
carregarArquivoGLB('./2025.1_T2_Assets/', 'cacodemon', true, scene, 1);
function carregarArquivoGLB(caminho, nomeArq, visbilidadeInicial, scene, escala = 1) {
   var loader = new GLTFLoader();

   for (var i = 0; i < assetManager.num_total; i++) {
      loader.load(caminho + nomeArq + '.glb', function (gltf) {
         ...
         assetManager.cacodemon = obj;
         assetManager[nomeArq].num++;
...
function posicionar_cacodemons() {
   for (var i = 0; i < 1; i++) {

      let obj_cacodemon = assetManager.cacodemon;
