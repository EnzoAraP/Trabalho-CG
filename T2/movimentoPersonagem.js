 ...
 movimento(areas,fronteira,groundPlane,delta,moveForward,moveBackward,moveRight,moveLeft,moveUp,reset) {
   ...
   if (!isIntersectingGround && !isIntersectingStaircase && !intersectaPlataforma) {
      ...
      if (this.grandeArea >= 1) {
         //console.log(area);
         if (this.area != -1) {
            this.boxPersonagem = new THREE.Box3().setFromCenterAndSize(
               new THREE.Vector3(this.obj.position.x, this.obj.position.y - 0.9, this.obj.position.z),
               new THREE.Vector3(this.larg, 1.8, this.larg) // largura, altura, profundidade desejadas
            );
            if(this.grandeArea!=2)
               isIntersectingStaircase = this.raycaster.intersectObjects([areas[this.area].degraus[1].rampa, areas[this.grandeArea - 1].degraus[0].degraus[7]]).length > 0.00001;
            else
            ...
            for (var i = 0; i < 3; i++) {
               if (areas[this.grandeArea - 1].boundingCubos[i].intersectsBox(this.boxPersonagem)) {
