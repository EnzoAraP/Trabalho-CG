...
class Area4 {
    ...
    box_extras_area4() {
        for (let i = 0; i < this.blocosExtras.length; i++) {
            this.boundingBlocosExtras.push(new THREE.Box3().setFromObject(this.blocosExtras[i]));
        }
        for (let i = 0; i < this.esferas.length; i++) {
            console.log("Esfera!");
            this.esferas_box.push(new THREE.Box3().setFromObject(this.esferas[i]));
            this.caixas_esferas.push(new THREE.Mesh(new THREE.BoxGeometry(2 * this.raioEsferas, 2 * this.raioEsferas, 2 * this.raioEsferas), new THREE.MeshBasicMaterial()));
    ...
    colisoes_area4(speed, obj, largx, altura, largz, moveDir, contar_subida, area, naPlataforma_a4) {
        ...
        for (var i = 0; i < this.muralhas.length; i++) {
            ...
            let speedColisao = verifica_colisoes_com_blocos(obj, largx, altura, largz, moveDir, this.muralhas[i].box, speed, contar_subida);
            speed = speedColisao[0];

        }
        if (area == 3) {
            for (let i = 0; i < this.plataformas.length; i++) {
                if ((this.plataformas[i].em_movimento || !this.plataformas[i].subir) && !naPlataforma_a4[i]) {

                    let speedColisao = verifica_colisoes_com_blocos(obj, largx, altura, largz, moveDir, this.plataformas[i].box, speed, contar_subida);
                    speed = speedColisao[0];
            ...
            for (let i = 0; i < this.pontes_box.length; i++) {


                let speedColisao = verifica_colisoes_com_blocos(obj, largx, altura, largz, moveDir, this.pontes_box[i], speed, contar_subida);
                speed = speedColisao[0];
                colisaoComAPlataforma = speedColisao[1];
                if (colisaoComAPlataforma) {
                    console.log("colide");
            ...
            for (let i = 0; i < this.paredes_janelas_box.length; i++) {


                let speedColisao = verifica_colisoes_com_blocos(obj, largx, altura, largz, moveDir, this.paredes_janelas_box[i], speed, contar_subida);
                speed = speedColisao[0];
                colisaoComAPlataforma = speedColisao[1];
                if (colisaoComAPlataforma) {
                    console.log("colide");
            ...
            for (let i = 0; i < this.torres.length; i++) {
                ...
                let speedColisao = verifica_colisoes_com_blocos(obj, largx, altura, largz, moveDir, this.torres_box[i], speed, contar_subida);
                speed = speedColisao[0];
                colisaoComAPlataforma = speedColisao[1];
                if (colisaoComAPlataforma) {
                    console.log("colide");
    ...
    colisoes_blocos_esferas_area4(speed, obj, largx, altura, largz, moveDir, contar_subida) {
        for (let i = 0; i < this.blocosExtras.length; i++) {
            let speedColisao = verifica_colisoes_com_blocos(obj, largx, altura, largz, moveDir, this.boundingBlocosExtras[i], speed, contar_subida);
            speed = speedColisao[0];
        }

        for (let i = 0; i < this.esferas.length; i++) {
            let speedColisao = verifica_colisoes_com_blocos(obj, largx, altura, largz, moveDir, this.esferas_box[i], speed, contar_subida);
            speed = speedColisao[0];
            if (speedColisao[1])
                break;
        }
    ...
    colisoes_bala_area4(boxBala) {
        let colidiu = false;
        for (var i = 0; i < this.muralhas.length; i++) {
            colidiu = this.muralhas[i].box.intersectsBox(boxBala);
            if (colidiu)
                return true;
        }

        for (let i = 0; i < this.plataformas.length; i++) {
            colidiu = this.plataformas[i].box.intersectsBox(boxBala);
            if (colidiu)
                return true;
        ...
        for (let i = 0; i < this.pontes_box.length; i++) {
            colidiu = this.pontes_box[i].intersectsBox(boxBala);
            if (colidiu)
                return true;

        }
        for (let i = 0; i < this.paredes_janelas_box.length; i++) {
            colidiu = this.paredes_janelas_box[i].intersectsBox(boxBala);
            if (colidiu)
                return true;
        ...
        for (let i = 0; i < this.torres.length; i++) {
            ...
            colidiu = this.torres_box[i].intersectsBox(boxBala);
            if (colidiu)
                return true;
        ...
        colidiu = this.assetManager.BigBenBox.intersectsBox(boxBala);
        if (colidiu)
            return true;


        for (let i = 0; i < this.blocosExtras.length; i++) {
            colidiu = this.boundingBlocosExtras[i].intersectsBox(boxBala);
            if (colidiu)
                return true;
        ...
        for (let i = 0; i < this.esferas.length; i++) {
            colidiu = this.esferas_box[i].intersectsBox(boxBala);
            if (colidiu)
                return true;
